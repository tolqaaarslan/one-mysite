...
// DÜZELTME: Soru işareti ve nokta bitişik olmalı (?.)
if (window.visualViewport) {
    window.visualViewport.addEventListener('resize', () => {
        const currentHeight = window.visualViewport.height;
        // const diff = lastHeight - currentHeight; // Bu satır bazen gereksiz olabilir ama kalsın

        // Klavye açıldı mı kontrolü için basit mantık
        // Eğer ekran boyutu ciddi oranda küçüldüyse (klavye kadar)
        if (currentHeight < lastHeight - 100) {
            document.documentElement.classList.add('keyboard-open');
            // Sayfanın yüksekliğini o anki görünür alana sabitle
            // Bu sayede inputlar klavyenin arkasında kalmaz
            document.body.style.height = `${currentHeight}px`;
        }
        // Klavye kapandı (Ekran eski boyutuna döndü)
        ...
        // setRealVH fonksiyonu yukarıda tanımlıysa çağır
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.player').forEach((playerDiv) => {
        ...
        Object.assign(ph.style, {
            ...
            borderRadius: getComputedStyle(container).borderRadius || '8px'
        });
    ...
    // --- 1. ŞİFRE GÖSTER / GİZLE (Göz İkonu) ---
    const passwordToggles = document.querySelectorAll('.toggle-password');
    passwordToggles.forEach(icon => {
        icon.addEventListener('click', (e) => {
            // Tıklamanın başka yerleri etkilemesini engelle
            e.preventDefault();
            e.stopPropagation();

            // İlgili input alanını bul
            const wrapper = icon.closest('.floating-wrapper');
            const input = wrapper.querySelector('input');

            // Tipi değiştir (password <-> text)
            if (input.getAttribute('type') === 'password') {
                input.setAttribute('type', 'text');
                icon.classList.remove('fa-eye');
                icon.classList.add('fa-eye-slash'); // Çizgili göz
                icon.style.color = '#ffa500'; // Aktif renk
            } else {
                input.setAttribute('type', 'password');
                icon.classList.remove('fa-eye-slash');
                icon.classList.add('fa-eye'); // Normal göz
                icon.style.color = ''; // Rengi sıfırla
            }
        });
    });

    // --- 2. İPUCU GÖSTER / GİZLE (Ampul İkonu) ---
    const hintToggles = document.querySelectorAll('.toggle-hint');
    hintToggles.forEach(icon => {
        icon.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            ...
            // Görünürlüğü kontrol et ve değiştir
            // Computed Style kontrolü daha güvenlidir
            const isVisible = window.getComputedStyle(hintText).display !== 'none';
            ...
            if (isVisible) {
                ...
                icon.style.color = '#ffd700'; // Sarı renk (Ampul yandı)
            }
        });
    });
});
...
// --- HEADER KÜÇÜLTME EFEKTİ (SAF JS VERSİYONU) ---
const headerElement = document.querySelector('.site-header');

if (headerElement) {
    window.addEventListener('scroll', () => {
        // Eğer 50px'den fazla aşağı kaydırılırsa 'shrink' sınıfını ekle
        if (window.scrollY > 50) {
            headerElement.classList.add('shrink');
        } else {
            // Yukarı çıkılınca sınıfı kaldır
...
/* ============================================
   GELİŞMİŞ GÜVENLİK SİSTEMİ (DÜZELTİLMİŞ)
   ============================================ */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    securityForms.forEach(form => {
        form.addEventListener('submit', async (e) => {
            ...
            const targetSide = form.getAttribute('data-target');
            const btn = form.querySelector('.unlock-btn');
            const authBox = form.closest('.auth-box');
            const errorMsg = authBox ? authBox.querySelector('.error-msg') : null;

            // UI Elementleri
            const authOverlay = document.getElementById(`auth-form-${targetSide}`);
            const secretContent = document.getElementById('secret-content');
            const defaultContent = document.querySelector('.end-container-default');
            const navLines = document.querySelector('.text-navigation');
            ...
            try {
                // CLOUDFLARE WORKER URL'İNİ BURAYA YAZ
                const WORKER_URL = "https://one.tolqaarslan.workers.dev/";
                ...
                if (result.success) {
                    ...
                    // 3. Navigasyon Çizgilerini Gizle (İsteğe bağlı, temiz görünüm için)
                    // if(navLines) navLines.style.display = 'none'; 

                    // 4. Gizli İçeriği Göster
                    if (secretContent) {
                        secretContent.innerHTML = result.html;
                ...
                if (errorMsg) {
                    ...
                    errorMsg.textContent = "Cevaplar yanlış veya sunucu hatası.";
                }
...
window.openAuth = function (side) {
    // Önce diğer tüm açık pencereleri kapat
    closeAuth();

    const overlay = document.getElementById(`auth-form-${side}`);
    if (overlay) {
        overlay.style.display = 'flex';
        // Z-Index'i en üste taşı
...
// Formu Kapat (Vazgeç Butonu veya Boşluğa Tıklama)
...
// GİZLİ İÇERİĞİ KAPAT VE BAŞA DÖN (GÜÇLENDİRİLMİŞ VERSİYON)
window.closeSecret = function () {
    const secretContent = document.getElementById('secret-content');
    const defaultContent = document.querySelector('.end-container-default');
    const navLines = document.querySelector('.text-navigation');

    // 1. Gizli İçeriği Yok Et
    if (secretContent) {
        secretContent.classList.remove('visible'); // Animasyonu bitir

        // CSS transition süresi (300ms) kadar bekle sonra tamamen kaldır
        setTimeout(() => {
            secretContent.style.display = 'none';
            secretContent.innerHTML = ''; // İçini temizle (Video/Resim belleğini boşalt)
        }, 300);
    }

    // 2. Varsayılan İçeriği GERİ GETİR (Zorla Resetle)
    if (defaultContent) {
        // Önce display: block yap ki tarayıcı render etsin
        defaultContent.classList.remove('hidden');
        defaultContent.style.display = 'block';
        defaultContent.style.opacity = '0'; // Önce görünmez olsun (animasyon için)

        // Minik bir gecikmeyle görünür yap (Fade-in efekti için şart)
        requestAnimationFrame(() => {
    ...
    // 3. Navigasyon Çizgilerini Geri Getir
    if (navLines) {
        navLines.style.display = 'flex';
        setTimeout(() => navLines.style.opacity = '1', 100);
    }
};

// Boşluğa tıklayınca formu kapatma (Overlay'e tıklama)
document.querySelectorAll('.auth-overlay').forEach(overlay => {
...
// --- GELİŞMİŞ GÜVENLİK SİSTEMİ (ÇİFT TARAFLI) ---

// Formları seç (Hem sol hem sağ formu kapsar)
const securityForms = document.querySelectorAll('.security-form');
...
securityForms.forEach(form => {
    form.addEventListener('submit', async (e) => {
        ...
        // Formun hangi tarafa ait olduğunu al (HTML'deki data-target="left" veya "right")
        const targetSide = form.getAttribute('data-target');

        // İlgili buton ve hata mesajı kutusunu bul
        const btn = form.querySelector('.unlock-btn');
        ...
        // WORKER'A GÖNDERİLECEK VERİ PAKETİ
        const payload = {
            target: targetSide, // "left" veya "right"
            answers: answers // q1, q2... cevapları
        };

        try {
            // BURAYA KENDİ WORKER ADRESİNİ YAZ
            const WORKER_URL = "https://one.tolqaarslan.workers.dev/";
            ...
            if (result.success) {
                ...
                // 2. Varsayılan Yazıyı Gizle (Animasyonlu)
                if (defaultContent) {
                    defaultContent.style.opacity = '0';
                    defaultContent.style.transform = 'translateY(-20px)';

                }

                // 3. Gizli İçeriği Göster
                if (secretContent) {
                    secretContent.innerHTML = result.html;
                    secretContent.style.display = 'block';

                    // Hafif gecikmeyle görünür yap
                    setTimeout(() => {
                        secretContent.classList.add('visible');
                    }, 100);
                }
            ...
            errorMsg.style.display = 'block';
            errorMsg.textContent = "Cevaplardan biri veya birkaçı yanlış.";

            // Inputları salla (Hata efekti)
            const inputs = form.querySelectorAll('input');
            inputs.forEach(input => {
                input.classList.add('shake-error');
                setTimeout(() => input.classList.remove('shake-error'), 500);
...
window.openAuth = function(side) {
    ...
    if(overlay) {
        ...
        // 3. Ekrana Getir (Ama henüz görünmez olsun - Opacity 0)
        overlay.style.display = 'flex';
        overlay.style.opacity = '0'; // Animasyon öncesi gizli
        overlay.style.zIndex = '9999';
        ...
        // 5. Animasyonu Başlat (Kullanıcıya göster)
...
window.closeAuth = function () {
    document.querySelectorAll('.auth-overlay').forEach(overlay => {
        overlay.style.opacity = '0';
        overlay.querySelector('.auth-box').style.transform = 'translateY(20px) scale(0.95)';
        setTimeout(() => {
            overlay.style.display = 'none';
...
function handleScrollIndicator() {
    ...
    // Check where the scroll is happening (body/html or .container)
    const container = document.querySelector('.container');
    let scrollTop, scrollHeight, clientHeight;

    if (container && getComputedStyle(container).overflowY === 'scroll') {
        // Desktop case (container scrolls)
        scrollTop = container.scrollTop;
        scrollHeight = container.scrollHeight;
        clientHeight = container.clientHeight;
    } else {
        // Mobile/Standard case (window scrolls)
...
// Add event listeners for scroll indicator
// 1. Window scroll (mobile)
window.addEventListener('scroll', () => {
    window.requestAnimationFrame(handleScrollIndicator);
});

// 2. Container scroll (desktop)
const containerForScroll = document.querySelector('.container');
if (containerForScroll) {
    containerForScroll.addEventListener('scroll', () => {
        window.requestAnimationFrame(handleScrollIndicator);
    });
}
...
// Initial check
document.addEventListener('DOMContentLoaded', handleScrollIndicator);
